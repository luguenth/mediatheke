...
6. You'll have to wait a while before the data got parsed. You can inspect the progress by running `docker-compose logs -f worker`. There will be an output like this:
```
...
| worker    | Runs Celery background jobs (Filmliste import, indexing, series detection) |
